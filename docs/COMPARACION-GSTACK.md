...
| Sync de memoria entre máquinas (repo git privado) | ✅ `[IMPLEMENTADA]` | `nucleo/sync.mjs` — push/pull de `.fabrica/` a un repo git privado. Los JSONL de repofibe son append-only (memoria.mjs, dominio.mjs) — merge trivial sin conflictos de contenido, solo concurrencia de appends que git resuelve automáticamente. Auth states se sincronizan como snapshots (reemplazo, no append). Escaneo de secretos (`secretos.mjs`) corre ANTES de push para evitar que credenciales se publiquen. El usuario configura el repo destino con `sync.mjs configurar <url>`. |
